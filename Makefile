...
ifeq ($(git_dirty),)
	ifneq ($(git_tag),)
		...
		go_ldflags += -X github.com/kongweiguo/spire-tenant/pkg/common/version.gittag=$(git_version_tag)
	endif
	ifneq ($(git_hash),)
		go_ldflags += -X github.com/kongweiguo/spire-tenant/pkg/common/version.githash=$(git_hash)
	endif
endif
...
%_spire_plugin.pb.go: %.proto $(protoc_bin) $(protoc_gen_go_spire_bin) FORCE | bin/protoc-gen-go-spire
	...
	$(E) PATH="$(protoc_gen_go_spire_dir):$(PATH)" $(protoc_bin) \
		-I proto \
		--go-spire_out=. \
		--go-spire_opt=module=github.com/kongweiguo/spire-tenant \
...
%_grpc.pb.go: %.proto $(protoc_bin) $(protoc_gen_go_grpc_bin) FORCE
	...
	$(E) PATH="$(protoc_gen_go_grpc_dir):$(PATH)" $(protoc_bin) \
		-I proto \
		--go-grpc_out=. --go-grpc_opt=module=github.com/kongweiguo/spire-tenant \
		$<
...
%.pb.go: %.proto $(protoc_bin) $(protoc_gen_go_bin) FORCE
	...
	$(E) PATH="$(protoc_gen_go_dir):$(PATH)" $(protoc_bin) \
		-I proto \
		--go_out=. --go_opt=module=github.com/kongweiguo/spire-tenant \
		$<
